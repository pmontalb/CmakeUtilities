include_guard()

if (NOT FLAGS_CPP_STANDARD)
    message(STATUS "Using C++17 standard as nothing specified")
    set(FLAGS_CPP_STANDARD 17)
else()
    message(STATUS "Using C++${FLAGS_CPP_STANDARD} standard")
endif()
set(CMAKE_CXX_STANDARD ${FLAGS_CPP_STANDARD})

if (MISC_USE_POSITION_INDEPENDENT_CODE)
    message(STATUS "Use PIC")
    set(CMAKE_POSITION_INDEPENDENT_CODE ON)
endif()

set(STL_DEBUG_FLAGS "")
if ((NOT CMAKE_BUILD_TYPE STREQUAL "Release") AND DEBUG_USE_STL_DEBUG)
    message(STATUS "Use STL debug")
    set(STL_DEBUG_FLAGS "-D_GLIBCXX_DEBUG -D_GLIBCXX_DEBUG_PEDANTIC")
endif()

set(RTTI_FLAGS "")
if (MISC_USE_RUNTIME_TYPE_INFO)
    message(STATUS "Use RTTI")
    if ("${CMAKE_CXX_COMPILER_ID}" STREQUAL "MSVC")
        set(RTTI_FLAGS "/GR")
    else()
        set(RTTI_FLAGS "-frtti")
    endif()
endif()

include(cmake/Options.cmake)
include(cmake/Configurations.cmake)
include(cmake/Warnings.cmake)
include(cmake/ArchitectureFlags.cmake)
include(cmake/OptimizationFlags.cmake)
include(cmake/DebugFlags.cmake)
include(cmake/SanitizerFlags.cmake)
include(cmake/CodeCoverageFlags.cmake)
include(cmake/IncludeWhatYouUse.cmake)


